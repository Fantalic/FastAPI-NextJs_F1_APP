...
interface IProps{
  ...
  searchText?:string
  onSelectRow: (rowIdx: number, data:any) => void
}
...
export default function Table(props:IProps) {
    ...
    const [sortedRows, setSortedRows] = useState<Record<string,any>[]>([]);
    ...
    useEffect(() => {
      const sortedRows = [...props.rows].sort((a, b) => {
        const valA = parseNumberOrString(a[sortCol]);
        const valB = parseNumberOrString(b[sortCol]);
      ...
      if(props.searchText) {
        const filteredRows = sortedRows.filter( row =>{
          return Object.values(row).some( value => value.toString().toLowerCase().includes(props.searchText.toLowerCase()))
        })
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              { sortedRows.map((row, rIdx) => (
                <tr
                    ...
                    onClick={()=>props.onSelectRow(row[ID_COL], row)}
                >
                  ...
                  {shownCols.map((col, colIdx) => (
                    ...
                    >
                      ...
                      { props.searchText && (
                        <HighlightedText text={row[col]} highlight={props.searchText} />
                      )}
                    </td>
                  ))}
                </tr>
